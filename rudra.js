...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        global.client.timeStart = new Date().getTime(),
            ...
            function() {
                const events = readdirSync(global.client.mainPath + '/Ayush/events').filter(event => event.endsWith('.js') && !global.config.eventDisabled.includes(event));
                for (const ev of events) {
                    try {
                        var event = require(global.client.mainPath + '/Ayush/events/' + ev);
